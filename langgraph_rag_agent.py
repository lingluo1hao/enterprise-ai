...
class LangGraphRAGApp:
    ...
    def _quick_classify(self, query: str) -> str:
        ...
        分类规则（按优先级）：
        1. 命中闲聊词（问候/感谢/身份/年龄/情绪/寒暄）且不含具体领域术语 → chitchat
        ...
        """
        q = query.lower()
        # 规则 1：闲聊关键词（问候 / 感谢 / 身份 / 年龄 / 情绪 / 日常寒暄）。
        # 这类问题无知识库答案，若按 simple 去检索会得到"未检索到"的生硬回答。
        # 防误伤：命中闲聊词的同时若含具体领域术语（协议/设备/参数/型号等），
        # 视为"打招呼+问正事"的复合句，放行到 simple/complex 走检索。
        # 注意领域词只能放具体技术术语，不可放"怎么/多少/如何"等宽泛疑问词，
        # 否则会把"今天天气怎么样/最近怎么样"这类闲聊误判为复合句。
        _domain_hint = any(w in q for w in [
            "协议", "设备", "参数", "型号", "功能", "接口", "指令",
            "信号", "报文", "配置", "心跳", "通讯", "波特率", "协议号",
            "定位", "设置",
        ])
        if any(w in q for w in [
            "你好", "谢谢", "再见", "hi", "hello", "你是谁", "你叫什么",
            "多大了", "多大", "年龄", "几岁", "岁数",
            "你怎么样", "最近怎么样", "在吗", "吃了没", "干嘛呢",
            "高兴", "难过", "喜欢", "讨厌", "累不累", "饿不饿",
            "今天天气", "你几岁", "你多大",
